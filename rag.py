...
class Kb :
    ...
    @staticmethod
    def split_content(content, max_length=config['config']['knowledge_base']['max_length']):
        chunks = []
    ...
    def encode(texts):
        embeds = []
        for text in texts:
            response = embeddings(model=config['config']['embeddings']['model'], prompt=text)
...
if __name__ == '__main__':
    ...
    rag = Rag(config['config']['rag']['model'], Kb(config['config']['knowledge_base']['filepath']))
    while True:
        q = input('Human: ')
        r = rag.chat(q)
